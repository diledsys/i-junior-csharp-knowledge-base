...
namespace StaffAccounting
{
    internal class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            while (isRunning)
            {
                ...
                {
                    case AddCommand:
                        StaffManager.AddEmployee(employeesByPosition, uniquePositions);
                        break;

                    case RemoveCommand:
                        StaffManager.RemoveEmployee(employeesByPosition, uniquePositions);
                        break;

                    case ShowCommand:
                        StaffManager.ShowAll(employeesByPosition, uniquePositions);
                        break;
    ...
    {
        public static void AddEmployee(Dictionary<string, List<string>> staffByPosition, HashSet<string> positions)
        {
            ...
            staffByPosition.Add(position, staffByPosition[position]);
            Console.WriteLine($"Сотрудник {fullName} добавлен на должность {position}.");
        }

        public static void RemoveEmployee(
            Dictionary<string, List<string>> staffByPosition, HashSet<string> positions)
        {
            Console.Write("Введите должность: ");
            string position = Console.ReadLine()?.Trim();
